...
SimStudy8 <- subset(summary_stats_SimStudy8, (N_actual >= 75 & N_actual <= 200) & nsites == 25 & ntraps == 5)
...
SimStudy8_pa <- subset(summary_stats_SimStudy8, (N_actual >= 75 & N_actual <= 200) & (nsites == 25) & ntraps == 3)
...
SimStudy8_N_actual <- subset(summary_stats_SimStudy8, (nsites == 25) & ntraps == 3)
...
SimStudy12_p <- subset(summary_stats_SimStudy12, N_actual == 500 & (nsites == 64) & ntraps == 5)

var_color <- "tau"

ggplot(data = SimStudy12_p, mapping = aes(x = p, y = N_mean, color = get(var_color))) +
  geom_point() +
  geom_hline(yintercept = 500, linetype = "dashed", color = "red") +
  ggtitle("Population Estimate vs. Capture Probability") +
  theme(plot.margin = margin(10, 10, 10, 10)) +
  xlab("Capture Probability") +
  ylab("Population Estimate") +
  guides(color = guide_legend(title = var_color)) +
  annotate("text", x = 0.4, y = 520, label = "Actual Population")
ggsave("SimStudy12_N_mean_vs_N_actual.png", device = "png", dpi = 300)
```
